MODULE block_gen
  ...
  SUBROUTINE blocks(x_min,      & ! Global Minimum
    ...
    REAL, INTENT(IN) :: x_min, x_max, delta_x
    REAL, DIMENSION(6), INTENT(OUT) :: block_min, block_max
    ...
    WRITE(UNIT=6,FMT=201) 1,block_min(1),block_max(1)

    DO i = 2,num_blocks
      block_min(i) = x_min + block_max(i-1)
      block_max(i) = block_min(i) + x_max/6.0

      WRITE(UNIT=6,FMT=201) i,block_min(i),block_max(i)
